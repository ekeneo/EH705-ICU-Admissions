...
This report is created by Ekene Olatunji, Catherine Nassralla, Victoria Chin, Basma Chamas, and Sajiya Somji for the eHealth 705 Statistics final project.
...
The ICU Admissions dataset consists of 200 observations with 21 variables. From these observations we found:
  ...
  3. Most of the variables are integers but from information about the data, most of the variables can be recoded to factor variables. Several of the variables have two to three levels categories and so while they are numerical, their means do not offer any meaningful information. 
...
Several tests required recoding some of our numerical variables into categories. For Age, bins were created using equal-width bins. Systolic was recoded into several blood pressure categories. HeartRate was also recoded into categories of bradycardia, tachycardia and elevated heart rate.
...
Recoding of the Systolic variable: 
```{r}
ICU <- 
  within(ICU, {
...
Binning of the HeartRate variable: 
```{r}
ICU <- 
  within(ICU, {
...
1) There are more male observations in the dataset than female.
...
5) More admitted patients had no chronic renal failure, no previous admissions to the ICU, fracture, CPR or cancer when admitted. Most admitted patients had  PO2 above or equal to 60, blood pH above or equal to 7.25, PCO2 below or equal to 45, and creatinine below or equal to 2. Many more of the observations were of White individuals. Due to the large difference between groups some of these variables were not further explored, and the focus of exploratory data analysis and understanding of distribution was on variables where values might have been prior to admission. These might aid clinicians who work in the ICU in understanding the likelihood of survival for individuals who are admitted before lab work has been ordered.

6) Most patients admitted were conscious, with slighly more patients being comatose than in a deep stupor if unconscious. 

7) Looking at histograms of the three numerical variables in the dataset, which were Age, Systolic and HeartRate, it could be guessed that if any of the distributions were to be normal, they would be Systolic and HeartRate. Age is very obviously not normally distributed. Systolic looks like it is centered around 140, and the mean confirms this as mentioned above. The mean of HeartRate seems to be centered around 100 and its mean confirms this visual estimate. 
 ...
 * For the Vital Status by Service, we can also observe that more patients died receiving medical service than surgical service.
 ...
 * For the Vital Status by Renal, there are more patients without history of chronic Renal failure and most of them lived, however, it is also observed that there seems to be an equal proportion in the Vital Status of patients who had a history with chronic Renal failure.
 * In the case of Vital Status by Infection, it can be observed that there are more cases of patients that lived than died regardless of if they have an infection or not. However, there are more cases of patients who probably had infections and died.
 * For the vital Status by CPR, it can be observed that there are more patients who were admitted to the ICU without prior CPR but there is the likelihood of patients dying with prior CPR.
 ...
 * In the case of Vital Status by Fracture, it can be observed that there are more cases of patients that lived than died regardless of if they have had a Fracture or not.    
 * For the Vital Status by PO2, it can be observed that there are more patients who were admitted to the ICU with a PO2 over 60 but there is the likelihood of patients dying with a PO2 under 60.
...
The chi squared value returned from this test is 11.52, which is not as far from the null hypothesized value as some of the other values generated from other tests. The p value of 0.00069 still indicated that the null hypothesis should be reject and that Sex is not uniformally distributed. There are more men in this dataset than females.   
...
The distribution of Type can be concluded to be non-uniform as the null hypothesis should be rejected based on the chi squared value produced and low p value. As seen in exploratory data analysis, there are more individuals who have had emergency admissions than elective admissions. This difference in distribution is statistically significant. Fewer individuals died when they were admitted to the ICU on an elective basis.  
...
__Tests of normality - HeartRate__: 
...
The mean of Age, referring to the descriptive statistics for this attribute, is about 58 years. Visually, it can be seen that the distribution of Age has two peaks, one around 20 years of age and one around 70 years of age. The majority of subjects admitted to the ICU seem to be between 40-80 years old. 
...
__Critical Values for Difference in Means__  
...
__H0__: the variances for admissions involving cancer and admission not involving cancer are equal  
...
__H0__: the variances for admissions involving infection and those that do not are equal  
__Ha__: the variances are different
...
Another association that was explored was between Age and Consiousness. All three categories for Consciousness overlap with peaks between 50 and 75 years of age. There is a distinct peak at around 50 for deep stupor, indicating that this age might be associated with deep stupor when related to ICU admissions. However, deep stupor also has a secondary peak at around 75, which makes it less clearly how this consciousness category relates to age in this dataset. 
...
__Difference in Means (ANOVA)__  

__H0__: the variances for Age are the same across all levels of consciousness  
__Ha__: the variances are different
...
__Testing Normality__  
__H0__: the several distributions are normally distributed  
__Ha__: the distributions are not normally distributed
...
__Testing for Homogeneity of Variances__  
__H0__: the variances are homogenous   
__Ha__: at least one of the variances is different from the others  
...
__Non-parametric Kruskal-Wallis test__  
...
The Kruskal-Wallis test produced the p-value = 0.6468, which is greater than 0.05. As such, we would retain the null hypothesis and conclude that the distributions are the same and that the means are likely the same.
...
__H0__: the variances for elective and emergency are equal  
__Ha__: the variances are different
...
__H0__: the variances for medical and surgical are equal  
__Ha__: the variances are different
...
__H0__: the variances for previous admission within the past 6 months and no previous admission within the past 6 months are equal  
__Ha__: the variances are different
...
Since the p-value = 0.05185 for testing the homogeneity of variances is greater than 0.05, we retain the null hypothesis with a 5% risk of a type 1 error and conclude that the variances for previous admission within the past 6 months and no previous admission within the past 6 months are equal. As such, the Student two sample t-test is used to analyze whether there was a significant difference in means.
...
__Difference in Means__  
__H0__: the variances for history of chronic renal failure and no history
__Ha__: the variances are different

__H0__: the means are equal  
__Ha__: the means are different

The following is the comparison of means and variances between the history of chronic renal failue and no history for Age.
...
__Difference in Means__  
__H0__: the variances for Males and Females are equal  
__Ha__: the variances are different

__H0__: the means are equal  
__Ha__: the means are different

The following is the comparison of means and variances between Males and Females for Age.
...
__H0__: there is no difference between the proportion of male and female patients that died, risk = 0.05  
__Ha__: there is a difference between the proportion of male and female patients that died
...
__H0__: there is no difference between the proportion of infection probable versus non-infection probable carrying patients that died, risk = 0.05  
__Ha__: there is a difference between the proportion of infection probable versus non-infection probable carrying patients that died
...
__H0__: there is no difference between the proportion of patients that died with a history of chronic renal failure versus without, risk = 0.05  
__Ha__: there is a difference between the proportion of patients that died with a history of chronic renal failure versus without
...
__H0__: there is no difference between the proportion of deaths in patients who recieved CPR upon admission versus patients who did not, risk = 0.05  
__Ha__: there is a difference between the proportion of deaths in patients who recieved CPR upon admission versus patients who did not
...
__H0__: there is no difference between the proportion of patients that died in the elective admission group versus the emergency admission group, risk = 0.05  
__Ha__: there is a difference between the proportion of patients that died in the elective admission group versus the emergency admission group
...
Since the p-value of 0.0001667 is less than 0.05, we will reject the null hypothesis that there is no difference between the proportion of patients that died in the 6 systolic blood pressure level groups. We can see from the sample estimates that the proportions are indeed different between the 6 groups, with the highest proportion in the hypotenstion range. The proportions in the elevated, normal, stage 1 hypertension, stage 2 hypertension and stage 3 hypertension groups are much lower and are quite close to each other. Thus, the hypotension group had a higher proportion of patients that died. 
...
# Detection of outliers of Integer Variables 
...
Our null hypothesis is that the coefficients are equal to 0, whereas our alternative hypothesis is that our coefficients are not equal to 0. 

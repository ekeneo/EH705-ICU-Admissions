...
This report is created by Ekene Olatunji, Caterine Nassralla, Victoria Chin, Basma Chamas, and Sajiya Somji for the eHealth 705 Statistics in eHealth final project.


# About the ICU Admissions Dataset
The ICU Admissions Dataset was obtained from Dr. Deal 

## Description 

The ICU Admission dataset contains observations of patients that were admitted to an adult intensive care unit (ICU). Using the principles taught during the eHealth 705 course, the attributes that make up this dataset were explored, analyzed and a logistic regression model was created. The response variable of this dataset was Status of patients after admission, that is, whether they lived or died after admission to ICU. A description of the variables is available below.
...
##Correlation
Below is the correlation, conducted via Goodman Kruskal Gamma and Goodman Kruskal's Lambda. As we can see from the gamma reading below, not only is it negative thus depicting a negative relationship, it is also nowhere close to -1, depicting a non-perfect inverse correlation. 

With regards to the lambda reading, while it is positive, thus depicting a positive relationship, it is also nowhere close to 1 like the gamma reading, thus depicting a non-perfect positive correlation. 
```{r}
pacman::p_load(DescTools)
ICUtable<-as.table(as.matrix(ICU))
GoodmanKruskalGamma(ICUtable, conf.level=0.95)

Lambda(ICU, direction = c("symmetric", "row", "column"), conf.level=0.95)
```

## Crosstabs (relations between categorical variables)
Below are all the crosstabs of each categorical variable with status as the Statistically significant crosstabs include: Creatinine(X2=45.878,p=0.000), Type(X2=10.527, p=0.001), CPR(X2=7.821, p=0.005), Infection(X2=5.759,p=0.016),Renal(X2=4.976, p=0.029), and Service(X2=5.981, p=0.014). 
```{r}
...
```{r}
ICU$Systolic<-cut(ICU$Systolic, breaks=c(0,80,119,129,256))
...
As we can see, key predictors that are shown to be statistically significant are age, cancer, type, pH, arterial blood gas concentration, and consciousness. We have confidence in our model due to the large difference between the null and residual deviance. Some of our confidence intervals have zero in them, leading us to have some doubt in our model. 
```{r}
...
### Final model 
Below is the final model, reduced to its significant components. As we can see below,all the predictors are statistically significant, leading us to reject the null hypothesis that they are equal to 0. The large difference between the null and residual deviance lends confidence in our model. 
```{r}
ICU.final.m<-glm(formula=Status~Age+Cancer+PH+PCO2+Consciousness+Type, family=binomial(logit),data=ICU)
summary(ICU.final.m)
confint(ICU.final.m)
exp(coef(ICU.final.m))
```

However, we must also evaluate just how good our model is, which is why we have decided to split the dataset into training and testing. The code below splits the ICU dataset into training and testing, with 150 observations in the former and 50 in the latter. 
```{r}
...
```
Below is the logistic regression conducted on the on the training model, using the reduced final model. As we can see, all predictors are statistically significant, with the gap in the null and residual deviances providing confidence in our model. 
```{r}
ICU.tr<-glm(Status~Age+Cancer+PH+PCO2+Consciousness+Type, family=binomial(logit),data=train)
summary(ICU.tr)
```

### Testing & training the model
Below is the code for using the training model to predict the values in the testing sample, with the output showing probabilities and predictions.  The sample below predicts "dead". 
```{r}
...
Below we assessed whether there was multicollinearity in the dataset using VIF. As we can see, the VIFs are less than 4, thus demonstrating that there is no multicollinearity in the dataset. 
```{r}
pacman::p_load(car)
car::vif(ICU.tr)
```
Below is our attempt to test the null model. As we can see, the intercept is statistically significant, thus demonstrating that our model is worth pursuing, although the deviances are identical. 
```{r}
fit0<-glm(Status~1, family=binomial(logit), data=train)
summary(fit0)
```
Below is our null model with all the variables fitted. As we can see, all the predictors are statistically significant and depict an explanation in the variance in the variables. 

```{r}
fitall<-glm(Status~Age+Cancer+PH+PCO2+Consciousness+Type, family=binomial(logit), data=train)
summary(fitall)
```
Below is the Chi-square test for the predictors in the null model, which is too, statistically significant. 
```{r}
anova(fitall, fit0, test="Chisq")
```
We also used the Wald test to test the significance of our predictors. According to this test, all but PCO2 are statistically significant. 
```{r}
pacman::p_load(survey)
regTermTest(ICU.tr, "Age")
regTermTest(ICU.tr, "Cancer")
regTermTest(ICU.tr, "PH")
...
According to the classification and regression tress below, to predict whether a patient lived or died, both consciousness and systolic blood pressure played a role. We refused to further prune our tree as the results of our cp tree does not support such decision. 
```{r}
